/** @import {Jobs, Job, Database, VotiveConfig} from "./bundle.js" */
...
function shouldSkip(status) {
  ...
  const cooldownDays = Math.min(2 ** (status.failureCount - 1), 32)
 ...
 */
function buildPendingFetch(job, processor, config, database) {
  return {
    url: job.data,
    ...
    async run() {
      ...
      try {
        response = await fetchWithDefaults(job.data, config)
      } catch (e) {
        database.url.recordFailure(job.data)
        return
      }
      ...
      const redirect = redirectFor(response, job.data)

      try {
        const data = await response[job.runner]()
        const parsed = processor.fetcher(data)
        database.url.create(job.data, parsed, job.destination, { redirect })
      } catch (e) {
        console.error(e)
        database.url.recordFailure(job.data)
      }
...
async function fetchURLs(urls, config, database) {
  const processors = config.plugins.flatMap(plugin => plugin.processors
    ?.flatMap(processor => processor.read?.url && { fetcher: processor.read.url, extensions: processor.extensions }))
    .filter(a => a)
  ...
  for (const job of urls) {
    ...
    const processor = processors.find(p => p.extensions.includes(job.extension))
    if (processor) pending.push(buildPendingFetch(job, processor, config, database))
  }

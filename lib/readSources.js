...
import { splitURL, pageSettingsFolder } from "./utils/index.js"
import createPluginAPI from "./pluginAPI.js"

/** @import {VotiveConfig, VotivePlugin, VotiveProcessor, FlatProcessors, Abstracts, Abstract, UrlTasks, ProcessorExtension, Router, PluginAPI} from "./bundle.js" */
...
/**
 * Stands in for the real PluginAPI while read() is running - read()'s
 * final target isn't settled until it returns (it can override its own
 * routed path via a returned `filePath`), so every api.* call it makes
 * here is queued instead of executed. dispatchAccumulatedCalls() below
 * replays them for real once the final path is known.
 *
 * A queued call can't return real data (there's nothing to read yet
 * against the right dependent) - target()/targets() return undefined/[]
 * rather than throwing, matching what a not-yet-existing target already
 * looks like via the real API. A readFile() that needs to *use* another
 * target's data synchronously to build its own return value can't do
 * that through this accumulator; only fire-and-forget calls (read-
 * tracking a target, creating one, linking a URL) are supported.
 * @returns {{ api: PluginAPI, calls: { method: string, args: any[] }[] }}
 */
function createAccumulatorAPI() {
  ...
  return {
    api: {
      target(filePath) {
        record("target", [filePath])
        return undefined
      },
      targets(params) {
        record("targets", [params])
        return []
      },
      createTarget(target) {
 ...
 * every call attributes its dependency tracking to `dependent`, the
 * target's actual final path, not the routed path read() started with.
 * @param {Database} database
...
function dispatchAccumulatedCalls(database, dependent, calls) {
  ...
  for (const { method, args } of calls) {
    if (method === "target") touchAllProperties(api.target(...args))
    else if (method === "targets") api.targets(...args).forEach(touchAllProperties)
    else if (method === "createTarget") api.createTarget(...args)
    else if (method === "url.get") api.url.get(...args)
    else if (method === "url.create") api.url.create(...args)
  }
}

/**
 * target()/targets() only record a dependency on properties actually
 * read off their result (see queries.dependency.track in
 * createDatabase.js) - the original synchronous read that would have
 * done this never happened here, since the accumulator returned a
 * placeholder instead of real data. There's no way to know which
 * properties the plugin would have touched, so this touches all of
 * them, tracking the whole target rather than tracking nothing.
 * @param {{ abstract: any, data: any, metadata: Record<string, any> } | undefined} target
 */
function touchAllProperties(target) {
  if (!target) return
  void target.abstract
  void target.data
  for (const key in target.metadata) void target.metadata[key]
}
    ...
    async function process(plugin, processor, config) {
      ...
      if (filter.includes(sourceFileInfo.ext) && read) {
        ...
        if (format === "text") {
          ...
          const readDependent = targetFilePath
          const readSettings = database.setting.getByFolder(pageSettingsFolder(targetFilePath), readDependent)
          // read() might override its own target path (below), so its
          // final dependent isn't known until it returns - the real
          // PluginAPI can't be handed to it yet. Queue whatever api.*
          // calls it makes instead, and replay them for real once the
          // final path is settled (see dispatchAccumulatedCalls below).
          const { api: readAPI, calls: readAPICalls } = createAccumulatorAPI()

          const content = read(data, sourceFilePath, targetFilePath, readSettings, readAPI, config)
          if (content) {

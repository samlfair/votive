...
/** @import {VotiveConfig, VotivePlugin, VotiveProcessor, FlatProcessors, Abstracts, Abstract, Jobs, ProcessorExtension, Router} from "./bundle.js" */
      ...
      if (filter.includes(sourceFileInfo.ext) && read) {
        ...
        if (format === "text") {
          ...
          if (content) {
            ...
            const { jobs, abstract, metadata } = content
            ...
            allJobs && allJobs.forEach(job => job.extension = processor.extensions[0])
            updateSource()
            return {
              abstract,
              metadata,

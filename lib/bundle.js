...
/**
 * A file extension (e.g. ".html", ".md"), used to match a target/job
 ...
 * cached for it (see fetchURLs.js: `job.runner` picks which Response
 ...
 * @returns {{ abstract: Abstract, metadata: object, jobs?: Jobs }}
 */
 ...
 *   so a job created here can set its own `destination` (see Job) for
 *   dependency tracking - only passed when called as a `transformFile`
 *   hook (readAbstracts.js); absent for the `ProcessorRead.abstract`
 *   usage of this same callback type
...
/**
 * @typedef {AbstractWithExtension[]} AbstractsWithExtension
 */

/**
 * @typedef {object} AbstractWithExtension
 * @property {Abstract} abstract
...
/*
  @CLAUDE: I want you to show me an example of how a `Job` would
 ...
 *   processor's extensions produced this job, so fetchURLs.js can match
 *   it against a plugin's registered read.url handler
 */
...
async function bundle(config, cache) {
  ...
  */
  ...
  if (sources.length) {
    ...
    // Jobs a plugin has claimed are deferred the same way buffers are -
    // see `runFetches`, also returned alongside `database`.
    runFetches = (await fetchURLs([...sourcesJobs, ...abstractsJobs, ...foldersJobs], config, database)).runFetches

...
/**
 * Build and write destination files. `destination` is a stale target
 * (see Target) augmented with `buffer()`/`stream()` - lazy reads of its
 * `source` file path, for a copy-through processor (PDF, video, fonts, ...)
 * that just needs the original bytes without touching `node:fs` itself.
 * @callback ProcessorWrite
 * @param {Target & { buffer: () => Buffer, stream: () => import("node:fs").ReadStream }} destination
 * @param {Settings} settings
 * @param {PluginAPI} api
 * @param {VotiveConfig} config

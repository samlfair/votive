 ...
 * link a fetched URL. This also happens to be what makes readSources.js's
 * accumulator (queue every api.* call, replay it once the file's final
 * target path is known) unconditionally correct - every accumulated
 * call here is fire-and-forget, so there's no synchronous return value
 * a caller could have been relying on in the meantime.
...
/**
 * Read a file buffer by its path. No `settings`/read-capable `api` -
 * see ReadPluginAPI.
 * @callback ReadPath
...
/**
 * No `settings`/read-capable `api` - see ReadPluginAPI.
 * @callback ReadText
 * @param {string} text
 * @param {string} filePath

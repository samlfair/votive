 ...
 * @property {HandlePreviewRequest} [handlePreviewRequest]
 */

/**
 * Called by a dev server (voot) when serving a GET request for a target
 * whose extension this processor claims, letting a plugin mutate the
 * response body before it's sent - e.g. the html plugin injecting its
 * live-reload script. Only affects what's served over HTTP during
 * development, not what's written to disk or read back from the database;
 * a processor with no `handlePreviewRequest` is served as-is.
 ...
 * @returns {{ abstract: Abstract, metadata: object, urls?: UrlTasks, settings?: Settings }}
 */

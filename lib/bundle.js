 ...
 * }} url
 * @property {(filePath: string) => void} retarget - for a readFile() that's
 *   about to return a `filePath` override (see ReadTextResult/ReadPath):
 *   call as early as possible once the final path is known, so any
 *   target()/targets()/url.create() calls made afterward in the same
 *   readFile() attribute their dependency tracking to where the content
 *   is actually landing, not the routed path it started at. Calls made
 *   before retarget() can't be fixed retroactively - see pluginAPI.js.
 */

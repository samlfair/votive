...
function overlapExists(negate) {
  const exists = `
    EXISTS (
      ...
      WHERE m.target = d.path AND m.label = ft.label AND m.class = 'target'
...
function buildLevel(depth, maxDepth) {
  ...
  const fromClause = isDeepest
    ? `FROM filter_tree ft CROSS JOIN scoped_targets d`
    : `
      FROM filter_tree ft
      CROSS JOIN scoped_targets d
      LEFT JOIN filter_tree child ON child.parent = ft.id
      LEFT JOIN level_${depth + 1} child_eval ON child_eval.node_id = child.id AND child_eval.target = d.path`
...
function buildGetManySQL(maxDepth) {
  ...
  const statement = `
    ...
    scoped_targets AS MATERIALIZED (
      ...
      -- target in the whole database, not just the requested folder.
      ...
      SELECT path FROM targets WHERE dir = :folder OR dir LIKE :recursivePath
    ),
    ${levels.join(",\n")}
    SELECT dest.*, json_group_object(i.label, CASE WHEN i.type IN ('array', 'object') THEN json(i.value) ELSE i.value END) AS metadata
    FROM scoped_targets d
    INNER JOIN targets dest ON dest.path = d.path
    INNER JOIN metadata i ON dest.path = i.target AND i.class = 'target'
    JOIN level_0 root ON root.target = d.path AND root.node_id = 0 AND root.satisfied = 1
    GROUP BY dest.path
  `
 ...
 * targets joined to their metadata. Worth a dedicated statement rather

...
function overlapExists(negate) {
  const exists = `
    EXISTS (
      ...
      WHERE m.destination = d.path AND m.label = ft.label
...
function buildGetManySQL(maxDepth) {
  ...
  const statement = `
    ...
    ${levels.join(",\n")}
    SELECT dest.*, json_group_object(i.label, i.value) AS metadata
    FROM scoped_destinations d
    INNER JOIN destinations dest ON dest.path = d.path
    INNER JOIN metadata i ON dest.path = i.destination

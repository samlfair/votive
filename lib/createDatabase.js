...
function prepareStatements(database) {
   ...
   * @property {string} type - 'target' | 'folder' | 'folder_recursive' | 'url'
   */

/*
  @CLAUDE: can we make the `type` value in `SQLiteMetadata` an enum?
*/
   ...
   * @property {string} type - the JSON value's type (text/integer/array/...)
   * @property {string} class - 'target' | 'folder' | 'folder_recursive' | 'url'
  ...
  return {
    ...
    target: {
      ...
      /**
       * @callback SQLiteTargetGetManyEmpty
       * @param {{ folder: string, recursivePath: string }} params
       * @returns {SQLiteTarget[]}
       */

      getManyEmpty: /** @type {{all: SQLiteTargetGetManyEmpty}} */ (/** @type {unknown} */ (database.prepare(buildGetManyEmptySQL()))),
...
function createDatabase(databasePath = ".votive.db") {
  ...
  const queries = {
            ...
            get() {
              ...
              mutators.forEach(method => {
                Object.defineProperty(values, method, {
                  /*
                    @CLAUDE: Why isn't this enumerable? Also, do these mutator
                    functions introduce the possibility of a namespace collision?
                  */
                  enumerable: false,
                  value: (...args) => {
    ...
    target: {
      ...
      /*
        @CLAUDE: It looks like we have `getByFolder` and `getManyWithTrackers`,
        which are effectively the same thing except that `getManyWithTrackers`
        accepts query filters. I would prefer to have a single `getByFolder`
      */
       ...
       * by accessed property.
       * @param {TargetGetByFolderParams | undefined} params
       */
      getByFolder(params) {
        const { folder = "", recursive = false, dependent } = params || {}

        const recursivePath = [folder, "%"].filter(a => a).join("/")
        const results = prepared.target.getManyEmpty.all({
          folder,
        ...
        // Per-property lazy tracking, same mechanism getWithTrackers uses -
        // layered on top of the folder-level edge above. The folder edge
        // covers membership (a target appearing/disappearing has no prior
        // property read to have tracked); this covers content, so a
        // dependent that only reads e.g. `title` doesn't get staled by an
        // unrelated `views` change on the same target.
        ...
        /*
          @CLAUDE: Would this sort be faster if we run it in SQLite? Also, we
          shouldn't sort by date, since that's not a Votive concept (it's a 
          Vowel concept). If we're going to sort at all, we should use Votive
          concepts and (if preferable) run the logic in SQLite.
        */
        many.sort((a, b) => {
          if (a.metadata.date) {
            if (b.metadata.date) {
      ...
      getManyWithTrackers(params) {
        ...
        const recursivePath = [folder, "%"].filter(a => a).join("/")
        const scopeParams = { folder, recursivePath: recursive ? recursivePath : folder }

        const isEmpty = !query || Object.keys(query).length === 0
        const results = isEmpty
          ? prepared.target.getManyEmpty.all(scopeParams)
          : prepared.target
            .getManyByDepth(Math.min(depthOfFilter(query), MAX_FILTER_DEPTH))
    ...
    url: {
      ...
      /*
        @CLAUDE: We need to be consistent with language. Elsewhere we call the
        `destination` property `dependency`. Lets use one everywhere. Which one?
      */
      /**
        * @param {string} url
        * @param {string} data
      ...
      get(url) {
        /*
          @CLAUDE: Is the below parameter correct? (Also in getStatus)
        */
...
function createTables(databaseSync) {
  databaseSync.exec(`
    ...
    -- @CLAUDE: Save these kinds of descriptors for the JSDoc types.
    -- type: what kind of thing destination identifies here -
    -- 'target' (a specific destination path, the historical behavior),
    -- 'folder' (exact folder, non-recursive), 'folder_recursive' (folder
    -- and everything under it), or 'url' (destination is a URL string).
    ...
    -- @CLAUDE: See above.
    -- class: same target/folder/folder_recursive/url vocabulary as
    -- dependencies.type, but named separately since it means something
    -- different here - it's what destination identifies for THIS row
    -- (a target path, a folder path, or a URL), not a dependency edge.
    -- type is unrelated and predates this: it's the JSON value type
    -- (text/integer/array/...) captured at write time.
    -- Settings used to live in their own table, keyed by
    -- (destination, label, value, source) so multiple values could stack
    -- under one label; they're now folder-scoped metadata rows instead
    -- (class = 'folder_recursive'), unique on (destination, label) same
    -- as everything else here - the most recent value for a given label
    -- wins rather than accumulating. source tracks which source file
    -- wrote a row so it can be cleaned up if that source changes.

...
function prepareStatements(database) {
  ...
  return {
    ...
    target: {
       ...
       * @param {{ folder: string, recursivePath: string, limit: number }} params
       * @returns {SQLiteTarget[]}
       */
       ...
       * @type {(depth: number) => { all: (params: { filter: string, folder: string, recursivePath: string, limit: number }) => SQLiteTarget[] }}
       */
      getManyWithFilters: (() => {
        const cache = new Map()
        return (depth) => {
    ...
    target: {
      ...
      getByFolder(params) {
        ...
        if (orderBy) {
          ...
          // SQL couldn't apply the limit (see scopeParams above) since
          // sorting happens here, after the query - apply it now instead.
          if (typeof limit === "number") return many.slice(0, limit)
        }

...
/**
 * Records a dependency action instead of writing it immediately - see
 * pluginAPI.js. `action` is a thunk that performs the real write once
 * called with the actual final dependent; `record` just queues it.
 * @callback DependencyRecorder
 * @param {(dependent: string) => void} action
 * @returns {void}
 */
...
/**
 * getWithTrackers()/getByFolder() accept a dependent as either a plain
 * path (the common case - direct/internal callers where the dependent
 * is already settled) or an already-deferred DependencyRecorder
 * (pluginAPI.js's use, where it isn't settled yet - see pluginAPI.js).
 * Normalizing here means every actual write site downstream only ever
 * deals with one shape.
 * @param {string | DependencyRecorder} dependent
 * @returns {DependencyRecorder}
 */
function toRecorder(dependent) {
  if (typeof dependent === "function") return dependent
  return action => action(dependent)
}
  ...
  return {
    ...
    target: {
      ...
      create: /** @type {{get: SQLiteTargetCreate}} */ (/** @type {unknown} */ (database.prepare(`
        INSERT OR IGNORE INTO targets (path, dir, extension, stale, abstract, source, data, write)
        VALUES (?, ?, ?, 1, ?, ?, ?, ?)
        RETURNING *
      `))),
      ...
      get: /** @type {{get: SQLiteTargetGet}} */ (/** @type {unknown} */ (database.prepare(`
        ...
        SELECT joined.path, joined.dir, joined.extension, joined.abstract, joined.source, joined.data, joined.write,
...
function createDatabase(databasePath = ".votive.db") {
  ...
  const queries = {
    ...
    dependency: {
      ...
      /**
       * Wraps `dependencyKey` so *reading* it records (via `record`,
       * doesn't write yet - see DependencyRecorder/pluginAPI.js) that
       * whatever ends up being the real dependent read this property of
       * `dependencyPath`. Untouched properties never get recorded at
       * all - this is what makes the tracking fine-grained (reading
       * `.abstract` doesn't record a dependency on `.data` too).
       * @param {object} dependencyFile
       * @param {string} dependencyKey
       * @param {any} dependencyValue
       * @param {string} dependencyPath
       * @param {DependencyRecorder} record
       */
      track(dependencyFile, dependencyKey, dependencyValue, dependencyPath, record) {
        Object.defineProperty(dependencyFile, dependencyKey, {
          enumerable: true,
          get() {
            record(dependent => prepared.dependency.create.get(dependencyPath, dependencyKey, dependent, "target"))
    ...
    target: {
      ...
      getWithTrackers(filePath, dependent) {
        const record = toRecorder(dependent)
       ...
       * @property {string | DependencyRecorder | undefined} [dependent] -
       *   a plain path (immediate) or a deferred recorder (pluginAPI.js's
       *   use) - see toRecorder.
      ...
      getByFolder(params) {
        ...
        const record = dependent ? toRecorder(dependent) : null
        ...
        if (record) {
          record(finalDependent => prepared.dependency.create.get(folder, "", finalDependent, recursive ? "folder_recursive" : "folder"))
        }
        ...
        // `dependency.track`'s getter unconditionally records a
        // dependency on access, so tracking with no dependent would try
        // to eventually flush an action with no real dependent to give
        // it.
        const many = results.map(({ abstract, metadata, data, ...rest }) => {
          ...
          if (record) {
            queries.dependency.track(trackedTarget, "abstract", parsedAbstract, rest.path, record)
            queries.dependency.track(trackedTarget, "data", data, rest.path, record)
            for (const key in parsedMetadata) {
              queries.dependency.track(trackedTarget.metadata, key, parsedMetadata[key], rest.path, record)
            }
          } else {
    ...
    url: {
      ...
      create(url, data, dependent, { redirect, canonical } = {}) {
        ...
        if (dependent) queries.url.linkDependent(url, dependent)
      },

      /**
       * The dependent-specific half of create() (the edge + stale-mark),
       * split out so pluginAPI.js can store the URL immediately while
       * deferring only this part until the real dependent is known -
       * see DependencyRecorder/pluginAPI.js.
       * @param {string} url
       * @param {string} dependent
       */
      linkDependent(url, dependent) {
...
function createTables(databaseSync) {
  databaseSync.exec(`
    ...
    -- here: a source file with no router (route()'s "0" placeholder,
    -- see readSources.js) round-trips as the *integer* 0, not the string
    -- "0", the moment it's stored. Confirmed empirically: this crashed
  ...
  // fix keeps coercing "0" (route()'s no-router placeholder) to an
  // integer until it's deleted and rebuilt from scratch. Not a practical
  // problem for vowel's own CLI (wipes .votive.db every invocation), but
  // a real gap for any longer-lived database.
}

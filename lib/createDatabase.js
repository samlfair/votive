 ...
 *   writeDestinations.js's `destination.buffer()`/`.stream()` helpers.
 */
  ...
  return {
    ...
    target: {
      ...
      create: /** @type {{get: SQLiteTargetCreate}} */ (/** @type {unknown} */ (database.prepare(`
        INSERT OR IGNORE INTO targets (path, dir, extension, stale, abstract, source)
        VALUES (?, ?, ?, 1, ?, ?)
        RETURNING *
      `))),
      ...
      get: /** @type {{get: SQLiteTargetGet}} */ (/** @type {unknown} */ (database.prepare(`
        ...
        SELECT joined.path, joined.dir, joined.extension, joined.abstract, joined.source,
  ...
  const queries = {
    ...
    setting: {
      ...
      deleteBySource(source) {
        ...
        deleted.forEach(row => staleFolderDependents(prepared, row.target, row.label))
        return deleted
      },

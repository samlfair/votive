...
function createDatabase(databasePath = ".votive.db") {
  ...
  const queries = {
    ...
    target: {
       ...
       * @returns {TargetOutput | undefined}
       */
      getBySource(sourceFilePath) {
        const target = prepared.target.getBySource.get(sourceFilePath)
    ...
    url: {
      ...
      /**
        ...
        */
      create(url, data, dependent, options) {
        // options can arrive as `null` rather than omitted - e.g. a
        // readBuffer() accumulator call replayed after a round trip
        // through the buffer cache's JSON.stringify/parse, which turns
        // an omitted `undefined` array element into `null`. A default
        // parameter (`= {}`) only covers `undefined`, not `null`, so
        // this normalizes explicitly instead.
        const { redirect, canonical } = options || {}

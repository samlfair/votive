 ...
 * @property {() => Promise<{ abstract: Abstract, metadata: object, settings?: import("./bundle.js").Settings, accumulatedCalls?: { method: string, args: any[] }[] }>} run
 */
...
function readBuffers(sources, config, database) {
  ...
  const tasks = pending.map(source => ({
    ...
    async run() {
      ...
      // processes one source file in isolation, so it only gets a
      // write-only, queued api (see ReadPluginAPI in bundle.js) - nothing
      // it might read (another target, folder settings) is guaranteed to
      // exist yet. The queued calls are plain data, so they cache and
      // replay correctly whether this result came from a fresh run or a
      // cache hit - see the dispatchAccumulatedCalls call in runBuffers().
      const { api: readAPI, calls: accumulatedCalls } = createAccumulatorAPI()

      const result = await source.readBuffer(source.sourceFilePath, readAPI, config)
      const resultWithCalls = { ...result, accumulatedCalls }
      await writeCached(cacheDirectory, source.sourceFilePath, resultWithCalls)
  ...
  async function runBuffers() {
    await Promise.all(tasks.map(task => limit(async () => {
      ...
      const { abstract, metadata, settings, filePath: filePathOverride, write, accumulatedCalls } = result
      const targetFilePath = filePathOverride || task.targetFilePath
      const created = database.target.create({ path: targetFilePath, abstract, metadata, source: task.sourceFilePath, write })

      dispatchAccumulatedCalls(database, created.path, accumulatedCalls || [])
      ...
      database.source.create(task.sourceFilePath, created.path, task.source.lastModified)
    })))
  }

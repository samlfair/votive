...
function readBuffers(sources, config, database) {
  ...
  const tasks = pending.map(source => ({
    ...
    async run() {
      ...
      // A cache hit never called source.readBuffer(), so there's no
      // fresh api.*() reads to flush - readAPI stays absent below.
      if (cached) return cached

      const readDependent = source.targetFilePath
      const readSettings = database.setting.getByFolder(pageSettingsFolder(source.targetFilePath), readDependent)
      const readAPI = createPluginAPI(database)

      const result = await source.readBuffer(source.sourceFilePath, readSettings, readAPI, config)
      await writeCached(cacheDirectory, source.sourceFilePath, result)
      // readAPI is only attached to the *returned* object, not the
      // cached JSON on disk - it isn't serializable, and a future cache
      // hit correctly has nothing to flush anyway (see above).
      return { ...result, readAPI }
    }
  }))
  ...
  async function runBuffers() {
    await Promise.all(tasks.map(task => limit(async () => {
      ...
      const { abstract, metadata, settings, filePath: filePathOverride, write, readAPI } = result

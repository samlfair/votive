...
function readFolders(folders = [], config, database, processors) {
  ...
  const processed = folderProcessors.flatMap(({ processor, plugin }) => {
    ...
    const jobs = folders.flatMap(folder => {
      ...
      // processor.syntax never existed (processors only have .extensions,
      // an array) - this always set job.syntax to undefined. Fixed to
      // match the same processor.extensions[0] pattern readSources.js and
      // readAbstracts.js already use for the same purpose.
      jobs.forEach(job => job.extension = processor.extensions[0])
      if (targets) {
    ...
    rootFolder.jobs.forEach(job => job.extension = processor.extensions[0])
